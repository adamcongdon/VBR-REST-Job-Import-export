...
Describe 'Import-VbrProxies' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-prx-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'proxy1' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/proxies/import' {
            $null = Import-VbrProxies -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/proxies/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrProxies -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-prx-001'
        }
    }
}
