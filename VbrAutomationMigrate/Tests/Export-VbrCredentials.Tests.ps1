...
Describe 'Export-VbrCredentials' {
    ...
    Context 'request shape' {
        ...
        It 'sends POST to /api/v1/automation/credentials/export' {
            $null = Export-VbrCredentials -Token $script:token
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/credentials/export$'
        }
        ...
        It 'sends Content-Type application/json' {
            # Pester captures ContentType by parameter name; confirm via Should -Invoke filter.
            Should -Invoke -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -Times 0 -ParameterFilter { $true }
            $null = Export-VbrCredentials -Token $script:token
            Should -Invoke -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -Times 1 -ParameterFilter {
        ...
        It 'sends body {"names":["foo","bar"]} when -Names is supplied' {
            $null = Export-VbrCredentials -Token $script:token -Names @('foo', 'bar')
            $parsed = $script:capturedBody | ConvertFrom-Json -Depth 50
    ...
    Context 'return shape' {
        ...
        It 'returns a parsed PSCustomObject (not a raw string)' {
            $r = Export-VbrCredentials -Token $script:token
        ...
        It 'preserves nested objects beyond the default ConvertFrom-Json depth' {
            $r = Export-VbrCredentials -Token $script:token
            # New-DeepFakeExportResponse nests 8 levels of { inner = { ... } }.
