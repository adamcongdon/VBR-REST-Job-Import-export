...
Describe 'Export-VbrProxies' {
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/proxies/export' {
            $null = Export-VbrProxies -Token $script:token
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/proxies/export$'
        }
        ...
        It 'sends body {"names":["foo","bar"]} when -Names is supplied' {
            $null = Export-VbrProxies -Token $script:token -Names @('foo', 'bar')
            $parsed = $script:capturedBody | ConvertFrom-Json -Depth 50
    ...
    Context 'return shape' {
        It 'returns a parsed PSCustomObject (not a raw string)' {
            $r = Export-VbrProxies -Token $script:token
