...
Describe 'Import-VbrCloudCredentials' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-cc-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'cc1' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/cloudCredentials/import' {
            $null = Import-VbrCloudCredentials -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/cloudCredentials/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrCloudCredentials -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-cc-001'
        }
    }
}
