...
Describe 'Import-VbrCredentials' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-cred-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'cred1'; type = 'standard' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/credentials/import' {
            $null = Import-VbrCredentials -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/credentials/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrCredentials -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-cred-001'
        }
    }
}
