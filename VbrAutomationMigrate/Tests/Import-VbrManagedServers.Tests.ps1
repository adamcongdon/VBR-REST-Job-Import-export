...
Describe 'Import-VbrManagedServers' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-ms-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'vc1' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/managedServers/import' {
            $null = Import-VbrManagedServers -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/managedServers/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrManagedServers -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-ms-001'
        }
    }
}
