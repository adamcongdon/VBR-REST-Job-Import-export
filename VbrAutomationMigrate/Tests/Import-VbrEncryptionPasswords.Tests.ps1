...
Describe 'Import-VbrEncryptionPasswords' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-ep-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'ep1' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/encryptionPasswords/import' {
            $null = Import-VbrEncryptionPasswords -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/encryptionPasswords/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrEncryptionPasswords -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-ep-001'
        }
    }
}
