...
Describe 'Export-VbrManagedServers' {
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/managedServers/export' {
            $null = Export-VbrManagedServers -Token $script:token
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/managedServers/export$'
        }
        ...
        It 'sends body {"names":["foo","bar"]} when -Names is supplied' {
            $null = Export-VbrManagedServers -Token $script:token -Names @('foo', 'bar')
            $parsed = $script:capturedBody | ConvertFrom-Json -Depth 50
    ...
    Context 'return shape' {
        It 'returns a parsed PSCustomObject (not a raw string)' {
            $r = Export-VbrManagedServers -Token $script:token
