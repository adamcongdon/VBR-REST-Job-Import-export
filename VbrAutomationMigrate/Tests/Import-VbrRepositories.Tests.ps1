...
Describe 'Import-VbrRepositories' {
    ...
    BeforeEach {
        ...
        Mock -ModuleName VbrAutomationMigrate -CommandName Invoke-RestMethod -MockWith {
            ...
            return [pscustomobject]@{ id = 'sess-repo-001' }
        }
        $script:token = New-FakeVbrToken
        $script:spec = [pscustomobject]@{
            items = @([pscustomobject]@{ name = 'repo1' })
    ...
    Context 'request shape' {
        It 'sends POST to /api/v1/automation/repositories/import' {
            $null = Import-VbrRepositories -Token $script:token -Spec $script:spec
            $script:capturedMethod | Should -Be 'POST'
            "$script:capturedUri" | Should -Match '/api/v1/automation/repositories/import$'
        }
    ...
    Context 'return shape' {
        It 'returns an object exposing an Id property (the session id)' {
            $r = Import-VbrRepositories -Token $script:token -Spec $script:spec
            $r.id | Should -Be 'sess-repo-001'
        }
    }
}
